...
- `cmd/` — Cobra command tree. `fetch` subcommands (`osv`, `cve`, `kev`, `epss`, `all`) delegate to fetchers via the `fetcher.Fetcher` interface. `wisteria unify --cache-dir <path>` runs Stage 1-3 production pipeline (walker → unifier.Unify → writer.Write) and exits non-zero on the first parse / write error. Stage 4 (KEV/EPSS annotators) plugs in once #20 / #21 land.
- `cmd/debug/` — `wisteria debug` parent + one-file-per-subcommand inspection helpers. `debug index` prints walker.Index distribution (PrimaryIDs total, per-Kind entry counts, entries-per-PrimaryID histogram, per-ecosystem OSV counts) or, with `--id`, lists the IndexEntry paths under one PrimaryID (exits non-zero if not found). `debug unify --id <PrimaryID>` runs Stage 1 walker + Stage 2 partial merge (References + Severities only, per #16) and prints the resulting UnifiedAdvisory as JSON; Descriptions / Affected stay empty until #17. Subcommands inherit the persistent `--cache-dir` flag.
...
- `internal/unified/unifier/` — Stage 2 of the unified-advisory pipeline. `Unify(ctx, sourcesRoot, index)` is the production entrypoint: iterates PrimaryIDs in lex-sorted order, calls `MergePrimary` per ID, collects SourceIDs (every alias other than the PrimaryID, deduped + sorted), and aborts on the first parse failure. `UnifyBestEffort(..., logger)` is the debug counterpart: it logs and skips broken PrimaryIDs instead of aborting (`logger` may be nil → discarded). `MergePrimary(ctx, sourcesRoot, primaryID, entries)` does the field-level merge using pure helpers: `mergeReferences` (URL normalize + dedup + tag union + lex sort, §8.2), `mergeDescriptions` (parallel hold, sort by source priority → lang → input index, §8.3), `mergeSeverities` (priority-aware (Type,Vector) / (Type,Score) dedup with deterministic Vector → Score tie-breaker, §8.4), `mergeAffected` (parallel hold, sort by priority → Provenance.ID → input index, §8.5). CVE5 ADP containers (CISA Vulnrichment etc.) contribute their own Provenance with an `#adp:<shortName>` ID suffix, so CNA and ADP appear as parallel siblings. Vendor priority array is the §8.1 draft; final ordering + const-ification decided in #22. `PriorityRank` / `SourceTag` are exported so sibling packages (writer) can pick a primary Provenance among many. KEV / EPSS land in #20 / #21.
- `internal/unified/writer/` — Stage 3. `Write(ctx, cacheDir, records)` deletes `<cacheDir>/unified/` (after a guard: cacheDir non-empty + absolute + not `/`, outDir basename == `"unified"`, outDir is a regular directory or absent — symlinks / files refused) and writes one JSON per record: `CVE-YYYY-NNNN` → `unified/cve/<year>/<id>.json`, otherwise → `unified/standalone/<ecosystem>/<id>.json` where `<ecosystem>` is the highest-priority OSV provenance under the record (via `unifier.PriorityRank`). PrimaryID `:` and `/` are escaped to `_`. Per-file atomic via temp + rename.
