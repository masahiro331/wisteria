...
- `internal/fetcher/osv/` — OSV fetcher; reads `ecosystems.txt` then downloads each `{ecosystem}/all.zip`, skipping a curated default exclusion list (CVE→OSV conversion buckets `GIT`/`[EMPTY]`, dead projects `GSD`/`UVI`, container-vendor distros, out-of-scope distros/platforms — see `defaultExcludedEcosystems`). `--osv-exclude` on `fetch osv`/`fetch all` replaces the default (pass `""` to fetch everything). Downloads run in parallel via `errgroup` with a configurable cap (`--concurrency`, default 4); the first error cancels in-flight siblings. Each archive is unzipped in place and removed. Upstream's `[EMPTY]` ecosystem (the bucket OSV uses for ecosystem-less generic advisories) is renamed to `Generic` at download time via `localEcosystem` — the upstream URL path keeps `[EMPTY]/all.zip`, only the on-disk directory is renamed so downstream stages see `Generic` verbatim without bracket-escape pain.
