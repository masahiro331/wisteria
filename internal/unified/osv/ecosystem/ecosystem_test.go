...
func TestNewRecordPyPI_Fields(t *testing.T) {
	...
	if got.Ecosystem != osv.EcosystemPyPI {
		t.Errorf("Ecosystem = %v, want %v", got.Ecosystem, osv.EcosystemPyPI)
	}
...
func TestParse_DispatchesByEcosystem(t *testing.T) {
	const body = `{"id": "X-1"}`
	rec, err := Parse(osv.EcosystemPyPI, strings.NewReader(body))
	if err != nil {
	...
	if rec.Base().Ecosystem != osv.EcosystemPyPI {
...
func TestParse_UnknownEcosystem(t *testing.T) {
	if _, err := Parse(osv.Ecosystem(9999), strings.NewReader(`{}`)); err == nil {
